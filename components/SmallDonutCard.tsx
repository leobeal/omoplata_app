...
export const SmallDonutCard: React.FC<SmallDonutCardProps> = ({
  ...
  return (
    <View className="min-w-0 rounded-lg bg-secondary p-3">
      ...
      {subtitle && <ThemedText className="mt-0.5 text-sm opacity-60">{subtitle}</ThemedText>}
      ...
      <View className="mt-3 items-center">
        <View
            ...
            <G transform={`rotate(-90 ${size / 2} ${size / 2})`}>
              {segmentsWithOffsets.map((segment, index) => {
                const animatedDasharray = animationProgress.interpolate({
                  ...
                  outputRange: [
                    `0 ${circumference}`,
                    `${segment.strokeDasharray} ${circumference}`,
                  ],
                });
          ...
          <View className="absolute items-center">
            {centerValue && <ThemedText className="text-lg font-bold">{centerValue}</ThemedText>}
            {centerLabel && (
              <ThemedText className="mt-0.5 text-xs opacity-60">{centerLabel}</ThemedText>
            )}
          </View>
        </View>
      </View>
        ...
        <View className="mt-3 flex-row flex-wrap justify-center">
          {segmentsWithOffsets.map((segment, index) => (
            <View
              ...
              />
              <ThemedText className="text-xs opacity-80">{segment.label}</ThemedText>
            </View>
          ))}
        </View>
      )}
